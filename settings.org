...
#+begin_src emacs-lisp :tangle yes
  (defvar smd/packages-to-install
    '(use-package
       ...
       clojure-ts-mode                    ;emacs-30+ (treesit-available-p)
       ts
       cider
       origami
       paredit
...
[[http://blog.burntsushi.net/ripgrep/][ripgrep]] seems like a neat addition to the search tool arms race
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package lsp-ui
    :ensure t
    :config
    (setq lsp-ui-doc-enable nil           ;I think this is the one I don't want enabled?
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package treesit-fold
    :load-path "~/repos/github.com/emacs-tree-sitter/treesit-fold")
#+END_SRC
...
=company= makes autocompletion sort of work. =paredit= and
=rainbow-delimiters= make the parentheses pretty nice. =eldoc= shows you
the docstring of a function in the echo area (but with LSP, it gets
annoying sometimes)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package clojure-ts-mode
    ...
    :init (progn
            ...
            (add-hook 'clojure-ts-mode-hook #'lsp-mode)
            (add-hook 'clojure-ts-mode-hook #'projectile-mode)
            (add-hook 'clojure-ts-mode-hook #'display-line-numbers-mode)
    ...
    :config (setq
             ...
             ;;t would make the comment body not-syntax-highlighted, which I dislike
...
#+end_src
* Try AI?
OK, this tech might be great, it might be mediocre, and it might be a
vast container of highly potent fertilizer. How can we tell the
difference? Probably by trying it out.
